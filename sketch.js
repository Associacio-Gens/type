...
let isDragging = false; // Flag to check if the text is being dragged
let dragStartX = 0; // Initial mouse X position when the drag starts
let initialStartX = 0; // Original value of startX before dragging
let dragStartY = 0; // Initial mouse X position when the drag starts
let initialStartY = 0; // Original value of startX before dragging
...
function setup() {
  ...
  slider3Label.html(`<b>Mida</b><br>${fSize} pt`);
  drawText();
...
function mousePressed() {
  if (mouseX > startX && mouseX < startX + wordWidth) {
    ...
    dragStartY = mouseY; // Record where the drag started
    initialStartY = startY; // Save the original startX
  }
}
...
function drawText() {
  startY = height / 2 + 100 - offValue;
  drawingPoints = [];

  let tempOptions = {
    x: 0, // Just for measuring, position doesn't matter yet
    y: startY,
    fontSize: fSize,
  ...
  let tempLetters = textToShapes(font, msg, tempOptions);
  let allX = [],
    allY = [];

  for (let i = 0; i < tempLetters.length; i++) {
    let shapes = tempLetters[i];
  ...
  wordWidth = maxX - minX;
  startX = width / 2 - wordWidth / 2 - offValue;
  // Centered start position, updated for dragging
  if (isDragging) {
    startX = initialStartX + (mouseX - dragStartX); // Update startX during dragging
    startY = initialStartY + (mouseY - dragStartY); // Update startX during dragging
  }
...
function gotData(data) {
  ...
  slider1Label.html(`<b>Vent</b><br>${windSpeed}m/s`);
  if (perlinRadius > 150) {
